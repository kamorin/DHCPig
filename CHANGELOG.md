...
- **New `RUN_SUMMARY` (INFO), raised first on every run, in every mode**, including dry-run and
  the read-only scans. A two-column list — what was done, what came back — one row per phase, in
  run order:

      - Declared 12 other devices' leases done       ->  12 DHCPRELEASE sent, no ownership
                                                          proof required
      - Asked for those addresses back by name       ->  server gave us 4 of 12 (option 50)
      - Requested addresses to drain the pool        ->  412 held of 690 asked; stopped
                                                          early at 412 (nak_burst)

  Written for a security engineer who isn't a DHCP specialist: the left column is plain English
  and stays short enough to scan (a test caps it), and protocol names appear on the right as
  results rather than as something you must already know to read the row. The *why* behind these
  primitives is stated once, in the recommendation, instead of per step.
...
- **Evidence rendering is now list-aware** in both front ends: `{did, got}` pairs render as two
  aligned columns (CLI padding / an HTML table), any other list renders as indented bullets, and
  scalars plus empty lists keep the compact one-line form. Needed for the step list;
  `servers`/`sample_hosts` on existing findings read better for it too.
- 329 unit tests passing (up from 312); ruff clean.
